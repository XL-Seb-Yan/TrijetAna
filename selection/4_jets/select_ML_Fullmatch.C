...
void select_ML_Fullmatch(const TString samplename="Res1ToRes2GluTo3Glu_M1-1000_R-0p5", 
    ...
    TTreeReaderValue<ULong64_t> event = {fReader, "event"};
		...
		//for(UInt_t ientry=0; ientry<3; ientry++){
			...
			h_dijet_mass->Fill((j_sel_arr[1]+j_sel_arr[2]).M());
			
			// making trijet from the selected jets, 0,1 come from Res2 when 2 comes from Res1
			int index_j0[12]={1,0,0,1,2,0,2,0,1,0,0,1}; 
			int index_j1[12]={2,2,1,3,3,3,3,3,3,1,2,2}; 
			int index_j2[12]={0,1,2,0,0,1,1,2,2,3,3,3};
			
			int n_combine = 3;
			if(j_sel_index_arr.size() > 3)
				n_combine = 12;
			...
			for(int i=0; i<n_combine; i++){
				...
				int isMatch_j1 = 0;
				int isMatch_j2 = 0;
				if((j_sel_index_arr[j0] == matching_index_arr[1] && mindR_arr[1] < 0.4) || (j_sel_index_arr[j0] == matching_index_arr[2] && mindR_arr[2] < 0.4))
					isMatch_j0 = 1;
				if((j_sel_index_arr[j1] == matching_index_arr[1] && mindR_arr[1] < 0.4) || (j_sel_index_arr[j1] == matching_index_arr[2] && mindR_arr[2] < 0.4))
					isMatch_j1 = 1;
				if(j_sel_index_arr[j2] == matching_index_arr[0] && mindR_arr[0] < 0.4)
					isMatch_j2 = 1;
				gen_dijet_matched = isMatch_j0+isMatch_j1+isMatch_j2;
				if(gen_dijet_matched == 3){
					h_dR_gq0->Fill(mindR_arr[0]);
					h_dR_gq1->Fill(mindR_arr[1]);
					h_dR_gq2->Fill(mindR_arr[2]);
				}
